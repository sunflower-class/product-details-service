...
def generate_template_based_html(
    ...
    🎯 **핵심 지시사항**:
    1. **구조 보존**: 템플릿의 HTML 구조, CSS 스타일, 레이아웃은 완전히 보존
    2. **완전한 텍스트 교체**: 템플릿의 모든 텍스트를 실제 상품 정보로 100% 교체
    3. **실제 이미지만 사용**: 오직 제공된 허용된 S3 이미지 URL들만 사용
    4. **상품명 정확 추출**: 실제 상품 정보에서 상품명을 찾아서 제목으로 사용
    5. **관련없는 정보 금지**: 다른 상품 정보는 절대 포함하지 않음
    
    ❌ **절대 금지사항**:
    - 템플릿 예시 텍스트 그대로 유지 ("PREMIUM PRODUCT", "EXCEPTIONAL QUALITY" 등)
    - placeholder/더미 이미지 URL 사용 (placehold, example.com 등)
    - 템플릿에 있던 다른 브랜드/제품명 유지
    - 실제 상품과 관련없는 내용 추가
    
    ✅ **반드시 해야할 작업**:
    - 모든 제목 → 실제 상품명으로 교체
    - 모든 설명 → 실제 상품 특징으로 교체
    - 모든 이미지 → 제공된 S3 URL로 교체
    - 템플릿 예시는 한 글자도 남기지 않고 완전 교체
    ...
    **메인 이미지:** {product_image_url}
    {image_urls_str}
    
    📋 **작업 지시:**
    1. 위 참고 템플릿의 HTML 구조와 CSS 스타일은 그대로 유지
    2. 템플릿의 모든 텍스트를 실제 상품 정보로 완전히 교체
    3. 실제 상품명을 찾아서 모든 제목에 사용
    4. 실제 상품 특징과 설명으로 모든 내용 교체
    5. 제공된 이미지 URL들을 모든 img 태그에 적용
    6. 템플릿 예시 텍스트는 한 글자도 남기지 말고 모두 교체
    
    완성된 HTML 상세페이지를 생성하세요.
    """
