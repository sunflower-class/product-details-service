...
def get_concept_html_template(product_page: ProductPage) -> any:
    ...
    for idx, block in enumerate(concept_blocks):
        ...
        print('concept:', block.concept_style)
...
def check_html(block: any, html: str) -> bool:
    ...
    1. **블록 정보 초과:** 블록에 없는 정보가 HTML 코드에 포함되어 있으면 안됩니다. (다른 상품에 대한 설명 등..)

    2. **블록 정보 불일치:** 블록에 있는 정보와 HTML 코드에 포함된 정보가 불일치하면 안됩니다. 유사한 의미는 사용가능 하지만 완전 다른 정보가 되면 안됩니다. (현재 상품과 틀린 정보 등..)
...
def create_html_block(block: any, style: StyleConcept) -> str:
    ...
    * 만약 제품 이미지에 대한 프롬프트라면 프롬프트 앞에 "product:" 를 추가하고 그 이외의 프롬프트라면 그대로 사용하세요. 이미지 프롬프트는 최대한 자세하게 작성해야 합니다.
